...
# ============================================================================
# Two scenarios, both using INCREASING hazard trend:
#   1. Sicker switchers, treatment helps (HR=0.7)
#      Gap: +0.5 -> +1.5 -> +0.8
#   2. Healthier switchers, treatment harms (HR=1.5)
#      Gap: -0.3 -> -1.5 -> -0.5
#
# Layout per scenario: A,B (True), C (No pseudo-switch), D,E (pseudo-switch),
#                      F,G (PSM), H,I (IPTW)
# ============================================================================
...
# ============================================================================
# Function to generate KM figure for one scenario
# ============================================================================
...
generate_km_figure <- function(scenario_name, scenario_desc, true_hr,
  ...
  cat("True HR:", true_hr, "| Gap:", gap_baseline, "->",
      gap_at_switch, "->", gap_end, "\n")
  cat("============================================================\n")
  ...
  title_wrap <- wrap_elements(
    grid::textGrob(scenario_desc,
                   gp = grid::gpar(fontsize = 14, fontface = "bold"))
  )

  final_plot <- title_wrap / legend_wrap / main_plot +
    plot_layout(heights = c(0.06, 0.04, 1))
  ...
  ggsave(png_file, final_plot,
         width = 14, height = 6.5, units = "in", dpi = 300)

  cat("Plots saved to:\n")
  cat("  -", pdf_file, "\n")
  cat("  -", png_file, "(300 dpi)\n")
...
# ============================================================================
# Generate KM Figures for Both Scenarios
# ============================================================================

# Scenario 1: Sicker switchers, treatment helps (HR=0.7)
fig_sicker <- generate_km_figure(
  scenario_name = "Sicker_Helps",
  scenario_desc = "Sicker switchers, switching helps (true HR = 0.7)",
  true_hr = 0.7,
  gap_baseline = 0.5,
  gap_at_switch = 1.5,
  gap_end = 0.8,
  seed = 99,
  file_suffix = "_sicker_helps"
)

# Scenario 2: Healthier switchers, treatment harms (HR=1.5)
fig_healthier <- generate_km_figure(
  scenario_name = "Healthier_Harms",
  scenario_desc = "Healthier switchers, switching harms (true HR = 1.5)",
  true_hr = 1.5,
  gap_baseline = -0.3,
  gap_at_switch = -1.5,
  gap_end = -0.5,
  seed = 99,
  file_suffix = "_healthier_harms"
)

cat("\n============================================================\n")
cat("All KM curve figures generated.\n")
cat("============================================================\n")
