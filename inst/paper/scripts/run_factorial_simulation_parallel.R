...
run_factorial_parallel <- function(
  ...
  confounder_trajectories <- list(
    variable = c(gap_baseline = 0.2, gap_peak = 1.5, gap_end = 0),
...
if (interactive()) {
  ...
  # Save raw results
  saveRDS(results, "factorial_results_raw.rds")
  write.csv(results, "factorial_results_raw.csv", row.names = FALSE)

  # Summarize
  summary <- summarize_results(results)

  # Save summary
  saveRDS(summary, "factorial_results_summary.rds")
  write.csv(summary, "factorial_results_summary.csv", row.names = FALSE)
